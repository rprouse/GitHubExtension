...
namespace Alteridem.GitHub.Akavache
{
    ...
    public abstract class PersistentBlobCache : IBlobCache
    {
        ...
        {
            filesystem = filesystemProvider ?? Factory.Get<IFilesystemProvider>();

            if (filesystem == null)
            {
                throw new Exception("No IFilesystemProvider available. This should never happen, your NInject DependencyResolver is broken");
            }
            ...
            {
                log.FatalException("Couldn't create cache directory", ex);
            }
            ...
            flushThreadSubscription = actionTaken
                ...
                .Subscribe(_ => log.Debug("Flushing cache"));
        ...
        AsyncSubject<byte[]> WriteBlobToDisk(string key, byte[] byteData, bool synchronous)
        {
            ...
            Func<IObservable<byte[]>> writeResult = () => BeforeWriteToDiskFilter(byteData, scheduler)
                ...
                }, ex => log.WarnException("Failed to write out file: " + path, ex));
    ...
    {
        public CPersistentBlobCache(string cacheDirectory, IFilesystemProvider fs = null) : base(cacheDirectory, fs, System.Reactive.Concurrency.Scheduler.Default) { }
    }
}
